                            ...
                            if (attrs.allow == 'imageonly') {
                                ...
                                if (videoFiles.length > 0) {
                                    var promises = videoFiles.map(function (file) {
                                        return new Promise(function (resolve, reject) {
                                            ...
                                            video.onloadedmetadata = function () {
                                                ...
                                                // Only allow videos of exactly 20, 40, or 60 seconds
                                                if (duration !== 20 && duration !== 40 && duration !== 60) {
                                                    resolve({ file: file, error: 'Duration ' + video.duration.toFixed(1) + 's is invalid. Only 20s, 40s, or 60s videos are allowed' });
                                                } else {
                                                    resolve(null);
                                                }
                                            }
                                    ...
                                    Promise.all(promises).then(function (results) {
                                        ...
                                        if (errors.length > 0) {
                                            var msg = "Invalid video duration. Only 20s, 40s, or 60s videos are allowed: " + errors.map(function (e) { return e.file.name + ' (' + e.error + ')'; }).join(', ');
                                            raiseError(errors.map(function (e) { return e.file; }), 'DURATION_INVALID', msg);
                                            return;
                                        }
                                        scope.upload();
                                    });
