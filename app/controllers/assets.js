...
exports.createFiles = function (req, res) {
  ...
  else return rest.sendError(res, "There are no files to be uploaded");
...
exports.deleteFile = function (req, res) {
  ...
  async.series(
    [
      function (next) {
        fs.unlink(path.join(config.mediaDir, file), function (err) {
          if (err) next("Unable to delete file " + file + ";" + err);
          else next();
        });
      },
      function (next) {
        if (file.match(config.videoRegex) || file.match(config.imageRegex)) {
          Asset.findOne({ name: file }, function (err, dbdata) {
            if (dbdata && dbdata["thumbnail"]) {
              var imageThumbnailName = dbdata["thumbnail"].replace(
                "/media/_thumbnails/",
                ""
              );
      ...
      function (next) {
        Asset.remove({ name: file }, function (err) {
          if (err) util.log("unable to delete asset from db," + file);
          next();
        });
      },
      function (next) {
        if (file.match(config.videoRegex) || file.match(config.imageRegex)) {
          fs.unlink(thumbnailPath, function (err) {
            if (err) util.log("unable to find/delete thumbnail: " + err);
            next();
          });
        } else {
          next();
    ...
    function (err) {
      if (err) rest.sendError(res, err);
      else return rest.sendSuccess(res, "Deleted file successfully", file);
    }
  );
};
        ...
        function (next) {
          Asset.findOne({ name: oldName }, function (err, asset) {
            ...
            asset.save(function (err) {
              if (err) util.log("unable to save asset after rename," + oldName);
              next();
            });
          });
        },
    ...
    Asset.load(req.body.dbdata._id, function (err, asset) {
      if (err || !asset) {
        ...
        asset.save(function (err, data) {
          if (err) return rest.sendError(res, "Categories saving error", err);
...
exports.banAsset = function (req, res) {
  ...
  Asset.findOne({ name: file }, function (err, asset) {
    ...
    asset.save(function (err, data) {
      if (err) {
        return rest.sendError(res, "Error banning asset: " + err);
      }
      return rest.sendSuccess(res, "Asset banned successfully", data);
    });
  });
};
...
exports.unbanAsset = function (req, res) {
  ...
  Asset.findOne({ name: file }, function (err, asset) {
    ...
    asset.save(function (err, data) {
      if (err) {
        return rest.sendError(res, "Error unbanning asset: " + err);
      }
      return rest.sendSuccess(res, "Asset unbanned successfully", data);
    });
  });
};
...
exports.getCalendar = function (req, res) {
  ...
  fs.readFile(calFile, "utf8", function (err, data) {
    if (err || !data) return rest.sendError(res, "Gcal file read error", err);
...
exports.updateCalendar = function (req, res) {
  ...
  fs.readFile(calFile, "utf8", function (err, data) {
    ...
    exports.createAssetFileFromContent(calFile, data, function () {
      if (err) return rest.sendError(res, "Gcal file write error", err);
      else return rest.sendSuccess(res, "Successfully updated Email");
    });
  });
};
...
exports.createLinkFile = function (req, res) {
  ...
  async.series(
    [
      ...
      function (next) {
        require("./server-assets").storeLinkDetails(
    ...
    function (err) {
      if (err) return rest.sendError(res, "error in creating link file", err);
      else
