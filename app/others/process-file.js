              ...
              function (img_cb) {
                imageMagick(src)
                  ...
                  .write(
                    ...
                    function (err) {
                      console.log(
                        " imageKick write done: " + filename + ";error:" + err
                      );
              ...
              function (img_cb) {
                ...
                imageMagick(src)
                  ...
                  .write(src, function (err, op1) {
                    if (err)
                      console.log(
                        "Image resize error for : " + src + "  " + err
                      );
                    img_cb();
                  });
              },
              ...
              function (video_cb) {
                ...
                new FFmpeg({ source: filePath })
                  ...
                  .on("end", function (filenames) {
                    thumbnail =
                      "/media/_thumbnails/" + random + filename + ".png";
                    video_cb();
                  })
                  .takeScreenshots(
                    {
    ...
    function (err) {
      Asset.findOne({ name: filename }, function (err, data) {
        ...
        asset.save(function (err) {
          if (err) errorMessages = errorMessages.push(err);
          if (errorMessages.length > 0) {
            console.log(
              "store details(media processing) error messages: " + filename
            );
