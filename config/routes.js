...
router.post(
  "/api/files",
  upload.fields([{ name: "assets", maxCount: 10 }]),
  assets.createFiles
);
...
router.patch("/api/files/:file/ban", assets.banAsset);
router.patch("/api/files/:file/unban", assets.unbanAsset);
...
require("../app/controllers/licenses").getSettingsModel(function (
  err,
  settings
) {
